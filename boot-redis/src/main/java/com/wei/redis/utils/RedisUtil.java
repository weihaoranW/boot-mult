...
@Value("${spring.redis.default-database:0}")
private int default_db;

@Autowired
private JedisPool jedisPool;

public String xGet(String key) {

 return this.get(key, default_db);
}

//<T> T parseObject(String text, Class<T> clazz,
public <T> T xGet(String key, Class<T> clazz) {

 String s = this.get(key, default_db);
 //
 if (s == null || s.length() == 0) {
  return null;
 }
 T data = (T) JSONObject.parseObject(s, clazz);
 return data;
}
...
public <T> String xSet(String key, T data) {

 String s = JSONObject.toJSONString(data);
 return xSet(key, s);
}

public String xSet(String key, String value) {
