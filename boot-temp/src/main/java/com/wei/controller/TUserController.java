...
public List<?> selectAll() {
 ...
 //return new Result<>(0, codeMessage, users);
 return l;
}

@ApiOperation(value = "添加用户")
@PostMapping(value = "/add")
public Result<?> addUser(@RequestBody @Valid TUser user) {
 obj.save(user);
...
public Result<?> selectUserByName(@RequestParam String name) {
...
public Result<?> updateUser(@RequestBody @Valid TUser user) {
 boolean flag = obj.updateById(user);
 if (flag) {
  return new Result<>(0, "更新成功", null);
 } else return new Result<>(0, "更新失败", null);
}
